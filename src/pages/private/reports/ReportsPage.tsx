...
import { ITicketCategory, IOngoingEvent } from '../../../interfaces/firebase/IEvent';
...
export default function ReportsPage() {
  ...
  const { data: ongoingEvents } = useQuery({
    ...
    queryFn: async () => {
      const ongoingEvents: IOngoingEvent[] = await _eventService.getOngoingEvents();
      for (const event of ongoingEvents) {
        event.attendees = await _eventService.getAttendeesByEventId(event.id ?? '');
      }
          ...
          <Table
            ...
            columns={[
              ...
              { 
                title: 'Start Date', 
                dataIndex: 'startTime', 
                render: (date: any) => convertUnixToTimeText(date)
              },
              { 
                title: 'End Date', 
                dataIndex: 'endTime', 
        ...
        <Card className="col-span-1 md:col-span-2 lg:col-span-2">
          <div className="mb-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-gray-500">
                Showing {filteredOngoingEvents?.length} of {ongoingEvents?.length} events
              </span>
            </div>
          </div>
          <Table
            ...
            columns={[
              ...
              { title: 'Attendees', dataIndex: 'attendees', render: (attendees: IUser[]) => attendees.length },
              {
              ...
              },
              {
                ...
                render: (ticketCategories: ITicketCategory[]) =>
                  ticketCategories.reduce((total, category) => total + category.ticketSold, 0),
              },
            ]}
          />
        </Card>
